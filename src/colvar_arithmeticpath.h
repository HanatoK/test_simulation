    ...
    scalar_type *s, scalar_type *z)
{
    ...
    for (size_t i_frame = 0; i_frame < total_frames; ++i_frame) {
        exponents[i_frame] = std::exp(exponents[i_frame] - max_exponent);
        log_sum_exp_0 += exponents[i_frame];
        log_sum_exp_1 += i_frame * exponents[i_frame];
    }
    saved_exponent_sum = log_sum_exp_0;
    log_sum_exp_0 = max_exponent + std::log(log_sum_exp_0);
    log_sum_exp_1 = max_exponent + std::log(log_sum_exp_1);
    saved_s = normalization_factor * std::exp(log_sum_exp_1 - log_sum_exp_0);
    if (s != nullptr) {
        *s = saved_s;
    }
    if (z != nullptr) {
