...
// 10*gamma_x == gamma_y
void PCVBiasedSimulations3(const std::string& path_filename = "../data/path_new3.txt") {
  ...
  Reporter reporter(100, atoms, "PCV_10_100_b.traj");
  std::ofstream ofs_restraint_traj("PCV_restraint_10_100.dat");
  // ofs_restraint_traj << "# step s z restraint_energy\n";
  ofs_restraint_traj << fmt::format("#{:>10s} {:>12s} {:>12s} {:>12s}\n", "step", "s", "z", "restraint_energy");
  std::ofstream ofs_bias_traj("PCV_bias_10_100_pcv.dat");
  // ofs_bias_traj << "# step s r_s fb_s\n";
  ofs_bias_traj << fmt::format("#{:>10s} {:>12s} {:>12s} {:>12s}\n", "step", "s", "r_s", "fb_s");
  std::ofstream ofs_hill_traj("PCV_bias_10_100_pcv.hills");
  ...
  std::ofstream ofs_pcv_traj("PCV_bias_10_100_pcv.traj");
  ...
  // double3 frictions{10.0, 100.0, 100.0};
  const std::vector<double> friction_x(num_atoms, 10.0);
  const std::vector<double> friction_y(num_atoms, 100.0);
  ...
  simulation.runLangevinDynamics(
      ...
      [&](){
        ...
        bias.writeOutput("PCV_bias_10_100_step");
  ...
  bias.writeOutput("PCV_bias_10_100");
}

int main(int argc, char* argv[]) {
  if (argc < 2) PCVBiasedSimulations3();
  else PCVBiasedSimulations3(argv[1]);
  return 0;
}
